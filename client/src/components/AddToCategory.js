...
const AddToCategory = ({
  ...
  const editCat = (index, name) => {
    setIsEdit({ status: !isEdit.status, index: index });
    ...
    <>
      <Row className={isLoggedIn ? 'title_container mb-2' : 'title_container'}>
        <Col sm={1} style={{ marginTop: '.20rem' }}>
          <div>
            <AiOutlineRight onClick={closeCart} />
          </div>
        </Col>
        <Col sm={11}>
          <div>
        ...
        <ToastContainer position='top-start'>
          ...
          >
            <Toast.Body
              ...
              style={{ fontSize: '14px', padding: '.30rem' }}
            >
              {successMsg}
            </Toast.Body>
          </Toast>
      ...
      {isLoggedIn && (
        <Row className='align-items-center'>
          <Col sm={10} className={!showCategory ? 'opacity-1' : 'opacity-0'}>
            <Dropdown>
              ...
              <Dropdown.Menu>
                <ul
                  className={
                    categories.length === 0 ? 'display-n' : 'display-b'
                  }
                >
                  {categories &&
                    categories.map((category, index) => (
                      <li
                        key={index}
                        id={index}
                        className={`${!getCurState(index)}`}
                      >
                        {category.category_name === 'Main Wishlist' && (
                          <Col sm={10}>
                            <span
                              style={{ cursor: 'pointer' }}
                              onClick={initialDropValue}
                            >
                              Main Wishlist
                            </span>
                          </Col>
                            ...
                            <Row>
                              <Col sm={10}>
                                ...
                                {!getCurState(index) ? (
                                  <span
                                    onClick={() =>
                                      dropDownHandler(category.category_name)
                                    }
                                    style={{ cursor: 'pointer' }}
                                  >
                              ...
                              <Col sm={1} className='p-0'>
                                {!getCurState(index) ? (
                                  <img
                                    ...
                                    className='trash-btn'
                                    onClick={() =>
                                      editCat(index, category.category_name)
                                    }
                                  ...
                                  <img
                                    ...
                                    src='https://s3.amazonaws.com/cdn.myshopapps.com/iwish/drawer/saveCat.svg'
                                  />
                                )}
          ...
          <Col sm={2} className='text-center p-0' onClick={addToCategory}>
            {showCategory ? (
              <BiXCircle title='close category' />
            ) : (
              <BiPlusCircle title='Add to category' />
            )}
