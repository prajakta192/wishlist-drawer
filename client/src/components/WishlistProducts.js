...
const WishlistProducts = ({ isLoggedIn, product, id, paginationData }) => {
  ...
  return (
    <>
      <Row
        style={{
          borderBottom: '1px solid rgb(245,245,244',
          position: 'relative',
        }}
        className='pb-2 mb-3'
      >
        ...
        <Col sm={10}>
          <Row>
            <Col sm={10} className='align-items-center'>
              <Row className='align-items-center'>
                <Col sm={10}>
                  <span className='productTitle' title={product.title}>
                    {product.title}
                  </span>
                </Col>
              </Row>
            ...
            <Col sm={2} className='text-center'>
              <span
                className='fs-5'
                onClick={() =>
                  window.removeFromWishlist(product.id, product.variant.id)
                }
              >
                <RiDeleteBin5Line />
              </span>
                ...
                <Col sm={10}>
                  <Dropdown
                    className={`transferMenu ${
                      transferPrdctState.productState &&
                      product.variant.id === transferPrdctState.productIndex
                        ? 'display-b'
                        : 'display-n'
                    }`}
                  >
            ...
            {isLoggedIn && (
              <Col sm={2} className='text-center'>
                <span
                  className='fs-5'
                  onClick={() => {
                    transferProductHandler(product.variant.id);
                  }}
                >
                  <BiTransfer />
                </span>
          ...
          <Row>
            <Col sm={10}>
              <Row className='align-items-end'>
                ...
                <Col sm={8} className='p-0'>
                  <div
                    className={`${
                      !State.cartState && product.variant.id === State.cartIndex
                        ? 'display-b'
                        : 'display-n'
                    }`}
                  >
                    <div className='iwish_qty_box'>
                      <div className='qty_box'>
                        ...
                        <span className='iwishProQuantity'>
                          <strong>1</strong>
                        </span>
                        <span>
            ...
            <Col sm={2} className='text-center'>
              <span
                className='add_to_cart fs-5'
                onClick={() => cartHandler(product.variant.id)}
              >
